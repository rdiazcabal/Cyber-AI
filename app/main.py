    ...
    try:
        ...
        for finding in findings:
            ...
            events.append({
                "provider": "AWS",
                "service": "SecurityHub",
                "eventName": finding.get("Types", ["SecurityHubFinding"])[0],
...
def generate_sample_events_for_integration(integration: CloudIntegration) -> list[dict]:
    ...
    Azure/GCP remain as scaffold until their real connectors are implemented.
    """
    config = parse_integration_config(integration.config_json)
    provider = integration.provider.lower()

    if provider == "aws":
        return fetch_real_aws_events(config)
